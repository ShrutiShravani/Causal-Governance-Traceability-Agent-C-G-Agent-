...
def dummy_df():
    ...
    y_train=pd.Series({"default.payment.next.month":np.random.choice([0, 1], 10)})
    features = {
...
def test_confident_learning_output_shape(dummy_df):
    ...
    X_train,y_train,features = dummy_df
    mislabeled_df, threshold = run_confident_learning_audit(X_train,y_train,features)
...
def test_confident_learning_handles_single_class(dummy_df):
    ...
    X_train,y_train,features = dummy_df
    y_train["default.payment.next.month"] = 1  # Make target column single-class
    
    with pytest.raises(CGAgentException):
        run_confident_learning_audit(X_train,y_train,features)


def test_confident_learning_reproducibility(dummy_df):
    """Ensure results are deterministic with same seed and small dataset."""
    X_train,y_train,features = dummy_df
    mislabeled_df_1, _ = run_confident_learning_audit(X_train,y_train,features)
    mislabeled_df_2, _ = run_confident_learning_audit(X_train,y_train,features)
